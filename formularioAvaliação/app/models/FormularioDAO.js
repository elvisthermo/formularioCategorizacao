                ...
                let quest =  req.session._number;
                    switch(form.number) {
                        case '1':
                            ...
                            collection.update({_id:result[0]._id}, {$set:{quest1:form.quest1, number:form.number}});
                            res.render('index', { validacao:{},form:{},number:parseInt(form.number)+1, _id:req.session.usuario});
                            break;
                        case '2':
                            collection.update({_id:result[0]._id}, {$set:{quest2:form.quest2, number:form.number}});
                            res.render('index', { validacao:{},form:{},number:parseInt(form.number)+1, _id:req.session.usuario });
                            break;
                        case '3':
                            collection.update({_id:result[0]._id}, {$set:{quest3:form.quest3, number:form.number}});
                            res.render('index', { validacao:{},form:{},number:parseInt(form.number)+1, _id:req.session.usuario });
                            break;
                        case '4':
                            collection.update({_id:result[0]._id}, {$set:{quest4:form.quest4, number:form.number}});
                            res.render('index', { validacao:{},form:{},number:parseInt(form.number)+1, _id:req.session.usuario });
                            break;
                        case '5':
                            collection.update({_id:result[0]._id}, {$set:{quest5:form.quest5, number:form.number}});
                            res.render('index', { validacao:{},form:{},number:parseInt(form.number)+1, _id:req.session.usuario });
                            break;
                        default:
                            res.render('end');
